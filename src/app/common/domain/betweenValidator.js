function stringBetween(arg,floor =0,ceil=Infinity, kind='value'){
    ...
    throw new Error(`Validation error: ${kind} "${arg}" must be between ${floor} and ${ceil} characters long`)
}
...
function numberBetween(arg,floor =0, ceil=Infinity, kind='value'){
    ...
    throw new Error(`Validation error: ${kind} "${arg}" must be between ${floor} and ${ceil}`)
}
